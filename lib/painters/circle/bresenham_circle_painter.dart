...
class BresenhamCirclePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Call the Bresenham Circle algorithm
  ...
  void _drawCircle(Canvas canvas, Offset center, double radius, Paint paint) {
    ...
    int p = 3 - 2 * radius.toInt() * interval; // Initial decision parameter
    ...
    while (x < y) {
      ...
      if (p <= 0) {
        p = p + 4 * x + 6; // Decision to move horizontally
      } else {
        y -= interval; // Move vertically
        p = p + 4 * (x - y) + 10;
      }
